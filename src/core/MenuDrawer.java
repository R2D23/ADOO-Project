...
public class MenuDrawer{
    //Se definen las constantes para identificar el boton.
    //Estas constantes se usan en las clases Escucha, Menu y FigureMenu
    ...
    */
	...
	Graphics2D g2;
        int x = a.getBounds().x;
        int y = a.getBounds().y;
        int ancho = a.getBounds().width;//ancho del area en donde se va a dibujar
        int alto = a.getBounds().height;//alto del area en donde se va a dibujar
	try{
            switch(id){
                case TRIANGLE:
                {
                    g2 = (Graphics2D) g;
                    FileInputStream img = new FileInputStream("triangulo.png");//se obtiene el icono
                    BufferedImage in = ImageIO.read(img);//se carga la imagen
                    g2.drawImage(in, x + ancho/3, y + alto/3,ancho/2,alto/2, null);//se dibuja la imagen
